...
import { CancelOrderParams, Client, Side, sideBeet } from "@ellipsis-labs/phoenix-sdk";
...
const OrderView = ({
    ...
    const [isCancelOrderActionActive, setIsCancelOrderActionActive] = useState(false);
    ...
    const handleCancelOrderAction = async () => {
        ...
        if(enumeratedMarket && enumeratedMarket.spotGridMarket && walletState.connected) {
            ...
            try {
                ...
                let orderSequenceNumberBN = new BN(order.order_sequence_number.toString());
                ...
                green(<span>{`Order canceled `}<Link href={`https://solscan.io/tx/${response}`} target="_blank">{` ↗️`}</Link></span>, 3_000)
    ...
    return (
        <div className={styles.orderViewOuterContainer}>
            <div className={styles.orderViewContainer}>
                ...
                <div className={styles.columnNameRow}>
                    <div className={styles.cancelOrderButton} onClick={() => {
