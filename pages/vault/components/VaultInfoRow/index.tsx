...
const VaultInfoRow = ({
    ...
    const [l3UiBookState, setL3UiBookState] = useState({bids: [], asks: []} as L3UiBook);
    ...
    useEffect(() => {
        const populateL3UiBook = async() => {
            ...
            const freshBook = await getL3Book(vaultData.market_address, DEFAULT_ORDERBOOK_VIEW_DEPTH);

            setL3UiBookState(() => ({...freshBook}));
            // setIsL3UiBookLoaded(() => (true));
        }

        const intervalId = setInterval(populateL3UiBook, DEFAULT_ORDERBOOK_UPDATE_FREQUENCY_IN_MS);
