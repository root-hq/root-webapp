...
export const getMarket = async(
    ...
    try {
        ...
        if (response && response.data && response.data.length) {
          ...
          console.log(`Failed to retrieve data on spot grid market:"${marketAddress}`);
          return null;
        }
      } catch (err) {
        console.log(
...
export const getMarketForPhoenixMarket = async(
    ...
    try {
        ...
        if (response && response.data && response.data.length) {
          ...
          console.log(`Failed to retrieve data on phoenix market:"${phoenixMarketAddress}`);
          return null;
        }
      } catch (err) {
        console.log(
...
export const getAllMarkets = async(): Promise<SpotGridMarket[]> => {
    try {
        ...
        console.log(
          `Error fetching info on all spot grid markets`,
        );
      }
}